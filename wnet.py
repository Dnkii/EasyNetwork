...
class wnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = self.up1(x2, x1)#1*64*80*80*80
        x4 = self.down3(x3,x2)#1*128*40*40*40
        x = self.up1(x4, x3)#1*64*80*80*80
        x = self.con_last(x)#1*4*80*80*80
        return self.softmax(x)#1*4*80*80*80
...
if __name__ == '__main__':
    ...
    net = wnet(1,4,batch_norm=True, sample=False).to(device)
    #Generate network structure figure
    # from tensorboardX import SummaryWriter
    # with SummaryWriter(comment='W-Net') as w:
